...
// #include "\Users\denki\Documents\PlatformIO\Projects\nokia5110\src\runLuaGame.h"
#include <sstream>
#include <Arduino.h>
#include <FS.h>
#include <SD.h>
...
void pushDisplay(M5Canvas _tft){
  for(uint8_t x =0;x<84;x++){
    for(uint8_t y =0;y<48;y++){
    display.drawPixel(x,y,rgb565_to_grayscale(_tft.readPixel(x,y)));
    }
  }
  display.invertDisplay(true);//白黒を反転させる
  display.display();
  delay(100);
}
...
void setup()
{
  ...
  /* Initialize the nokia5110 Display*/ //------------------------------------
  display.begin();
...
if (elapsedTime >= 1000/fps||fps==-1) {
  ...
  if(isEditMode == TFT_SOUNDEDIT_MODE){
    ...
    if(patternNo<10)display.fillRect(0,76,16,8,TFT_BLACK);
    ui.showInfo( screen, 200, 112 );//ボタン情報、フレームレート情報などを表示します。
    display.setCursor(200,120);
    display.println(patternNo);
    ...
    if(enemyF){
      ...
      if(toolNo != 0){
        if(toolNo==1){//カラーパレット
          for(int j = 0; j<8; j++){
            for(int i = 0; i<2; i++){
              display.fillRect(i*16,j*16,16,16,gethaco3Col(j*2+i));
            }
          }
        }
